...
namespace Praga.PaisaKanaku.Core.DataAccess.Repositories.Setup
{
    public class ProductCategoryRepository : IProductCategoryRepository
    {
        ...
        public ProductCategoryRepository(ILogger<ProductCategoryRepository> logger, IDataBaseConnection db)
        {
            _logger = logger;
            _db = db;
        }

        public async Task<Response<ProductCategoryInfoDb>> GetProductCategoryInfoById(Guid productCategoryInfoId, Guid loggedInUserId)
        {
            Response<ProductCategoryInfoDb> response = new Response<ProductCategoryInfoDb>().GetFailedResponse(ResponseConstants.NO_RECORDS_FOUND);

            try
            {
                string spName = DatabaseConstants.USP_PRODUCT_CATEGORY_INFO_GET_BY_ID;

                DynamicParameters parameters = new();
                parameters.Add("@ProductCategoryInfoId", productCategoryInfoId, DbType.Guid);
                parameters.Add("@LoggedInUserId", loggedInUserId, DbType.Guid);

                var result = await _db.Connection.QueryAsync<ProductCategoryInfoDb>(spName, parameters, commandType: CommandType.StoredProcedure);
                return result != null ? response.GetSuccessResponse(result.FirstOrDefault()) : response;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error in ProductCategoryRepository.GetProductCategoryInfoById({@productCategoryInfoId}, {@loggedInUserId})", productCategoryInfoId, loggedInUserId);
        ...
        public async Task<Response<List<ProductCategoryInfoDb>>> GetProductCategoryInfoList(Guid loggedInUserId)
        {
            ...
            {
                _logger.LogError(ex, "Error in ProductCategoryRepository.GetProductCategoryInfoList({@loggedInUserId})", loggedInUserId);
                response = response.GetFailedResponse(ResponseConstants.INTERNAL_SERVER_ERROR);
                return response;
            }
        }

        public async Task<Response<Guid>> SaveProductCategoryInfo(ProductCategoryInfoDb productCategoryInfoDb, Guid loggedInUserId)
        {
            Response<Guid> response = new Response<Guid>().GetFailedResponse(ResponseConstants.FAILED);

            try
            {
                string spName = DatabaseConstants.USP_PRODUCT_CATEGORY_INFO_SAVE;

                DynamicParameters parameters = new();
                parameters.Add("@Id", productCategoryInfoDb.Id, DbType.Guid);
                parameters.Add("@Name", productCategoryInfoDb.Name, DbType.String);
                parameters.Add("@LoggedInUserId", loggedInUserId, DbType.Guid);
            ...
            {
                _logger.LogError(ex, "Error in ProductCategoryRepository.SaveProductCategoryInfo({@productCategoryInfoDb}, {@loggedInUserId})", productCategoryInfoDb.ToString(), loggedInUserId);
                response = response.GetFailedResponse(ResponseConstants.INTERNAL_SERVER_ERROR);
            }
