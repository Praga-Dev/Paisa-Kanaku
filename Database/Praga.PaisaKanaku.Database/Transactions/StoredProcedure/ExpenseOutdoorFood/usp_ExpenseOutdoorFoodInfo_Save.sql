CREATE PROCEDURE [Transactions].[usp_ExpenseOutdoorFoodInfo_Save]
	@Id UNIQUEIDENTIFIER,
...
BEGIN TRY
	...
	DECLARE @ExpenseOutdoorFoodInfoId UNIQUEIDENTIFIER = CASE WHEN (@Id IS NULL OR @Id = @EmptyGuid) THEN NEWID() ELSE @Id END;
	...
	BEGIN  	
		...
		WHERE [Id] = @ExpenseOutdoorFoodInfoId AND [RowStatus] = 'A'

		UPDATE [Transactions].[ExpenseOutdoorFoodInfo]
	...
	SET @Result = @ExpenseOutdoorFoodInfoId;
