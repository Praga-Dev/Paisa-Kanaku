CREATE PROCEDURE [Transactions].[usp_ExpenseGroceryInfo_Save]
	@Id UNIQUEIDENTIFIER,
...
BEGIN TRY
	...
	IF EXISTS(SELECT 1 FROM [Transactions].[ExpenseGroceryInfo] WHERE [Id] = @Id AND NOT [RowStatus] = 'A')
	BEGIN
	...
	DECLARE @ExpenseGroceryInfoId UNIQUEIDENTIFIER = CASE WHEN (@Id IS NULL OR @Id = @EmptyGuid) THEN NEWID() ELSE @Id END;
	...
	BEGIN  	
		...
		SELECT @OldExpenseAmount = [ExpenseAmount] FROM [Transactions].[ExpenseGroceryInfo] 
		WHERE [Id] = @ExpenseGroceryInfoId AND [RowStatus] = 'A'
	...
	SET @Result = @ExpenseGroceryInfoId;
